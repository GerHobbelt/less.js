...
less.Parser.importer = function (file, currentFileInfo, callback, env, resolvePath, inputToOutputPath) {
    var pathname, dirname, data, relativeFile
        newFileInfo = {
    ...
    relativeFile = resolvePath(file, env);
    file = resolvePath(file, env, { absolute: true});
    ...
    function parseFile(e, data) {
        ...
        // As per our rewrite, we need to translate the input path to the output path
        var outputFile = relativeFile;

        // Only rewrite the path if it actually hit any of our rewritings...
        if(relativeFile != originalFile){
            path.normalize(path.relative(inputToOutputPath(env.rootFileName), inputToOutputPath(relativeFile)));
        }
        var j = outputFile.lastIndexOf('/');
        ...
        if(newFileInfo.relativeUrls && !/^(?:[a-z-]+:|\/)/.test(outputFile) && j != -1) {
            var relativeSubDirectory = outputFile.slice(0, j+1);
