...
less.Parser = function Parser(env) {
    ...
    var imports = this.imports = {
        ...
        push: function (path, currentFileInfo, importOptions, callback) {
            ...
            if (less.Parser.importer) {
                less.Parser.importer(path, currentFileInfo, fileParsedFunc, env, resolvePath, inputToOutputPath)
            } else {
    ...
    var inputToOutputPath = env.inputToOutputPath;

    var resolvePath = (function(resolveModulePath, resolveAppExtensionsPath, corePath, appPath, registerDependency){
        var path = require('path');
        var fs = require('fs');
        var split, file;

        return function(filePath, env, opts){
            if (!opts){
                opts = { absolute: false };
            }
            var convertPath = function(filePath){
                if(opts.absolute){
                    return filePath;
                }
                return path.relative(path.dirname(env.rootFileName), filePath);
            };
            if (filePath.indexOf('#') > -1){
                split = filePath.split('#');

                if (split.length != 2){
                    process.stderr.write("Path " + filePath + " is invalid\n");
                    return filePath;
                }

                var module = split[0];
                var extensionPath = resolveAppExtensionsPath(module);
                file = split[1];

                var extensionFilePath = path.join(extensionPath, file);

                registerDependency(extensionFilePath);
                if (file.indexOf('!') != 0 && fs.existsSync(extensionFilePath)){
                    return convertPath(extensionFilePath);
                } else {
                    if(file.indexOf('!') == 0){
                        file = file.substring(1);
                    }
                    var modulePath = resolveModulePath(module);
                    var moduleFilePath = path.join(modulePath, "base", file);

                    registerDependency(moduleFilePath);
                    if (fs.existsSync(moduleFilePath)){
                        return convertPath(moduleFilePath);
                    }

                    var errStr = "Path " + filePath + " could not be found. Searched in\n\t";
                    if(extensionFilePath.indexOf('!') < 0){
                        errStr += extensionFilePath + "\n\t";
                    }
                    errStr += moduleFilePath + "\n";
                ...
                if (split.length != 2){
                 ...
                 var module = split[0];
                 var modulePath = resolveModulePath(module);
                 var moduleFilePath = path.join(modulePath, file);
                ...
                if(split[0] == 'core'){
                    var coreFilePath = path.join(corePath, split[1]);
                    if(!fs.existsSync(coreFilePath)){
                        process.stderr.write("Could not load file " + filePath + " tried\n\t" + coreFilePath + "\n");
                        return filePath;
                    }
                    registerDependency(coreFilePath);
                    return convertPath(coreFilePath);
                } else if(split[0] == 'app'){
                    var appFilePath = path.join(appPath, split[1]);
                    if(!fs.existsSync(appFilePath)){
                        process.stderr.write("Could not load file " + filePath + " tried\n\t" + appFilePath + "\n");
                        return filePath;
                    }
                    registerDependency(appFilePath);
                    return convertPath(appFilePath);
                } else {
                    process.stderr.write("Prefix " + split[0] + " is invalid.\n");
                    return filePath;
                }
            }
    ...
    })(env.resolveModulePath, env.resolveAppExtensionsPath, env.corePath, env.appPath, env.registerDependency);
            ...
            root.toCSS = (function (evaluate) {
                return function (options, variables) {
                    ...
                    var evaldRoot,
                        ...
                        evalEnv.rootFileName = env.rootFileName;
                        evalEnv.inputToOutputPath = inputToOutputPath;
                        evalEnv.resolvePath = resolvePath;
                        evalEnv.urlArgs = env.urlArgs;
